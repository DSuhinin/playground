...
func isResponseRetriable(resp *http.Response, err error) bool {
	...
	if err != nil {
		if circuitErr := errors.Cause(err, (*breaker.CircuitError)(nil)); circuitErr != nil {
			if circuitErr == breaker.ErrTimeout {
				return true
			}
		}
...
//
func newRequest(request *ClientRequest) (*http.Request, error) {

	req, err := http.NewRequest(request.Method, request.Route, bytes.NewReader(request.Body))
	if err != nil {
		return nil, err
	}
